...
public class JoyconManager: MonoBehaviour
{
    ...
    public List<Joycon> joyConList; // Array of all connected Joy-Cons

    public void InitManager()
    {
		int i = 0;

		joyConList = new List<Joycon>();
		bool isLeft = false;
		HIDapi.hid_init();
		...
		while (ptr != IntPtr.Zero) {
			...
			// Debug.Log (enumerate.product_id);
				if (enumerate.product_id == product_l || enumerate.product_id == product_r) {
					...
					joyConList.Add (new Joycon (handle, EnableIMU, EnableLocalize & EnableIMU, 0.05f, isLeft));
					++i;
				}
				ptr = enumerate.next;
			}
